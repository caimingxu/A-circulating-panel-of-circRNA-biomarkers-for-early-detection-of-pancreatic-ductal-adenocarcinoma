...
class <- c(rep("con",6),rep("treat",6))    #??Ҫ?޸?
...
p <- ggplot(gene, aes(x = logFC, y = -log10(P.Value), color = sig)) +
  geom_point(alpha = 0.6, size = 1) + scale_x_continuous(breaks = seq(-4,7,1))+scale_y_continuous(breaks = seq(0,16,2)) +  
...
ggsave('gene.png', p, width = 5, height = 6, dpi = 1200)
